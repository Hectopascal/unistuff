...
int tritest(double x0,double y0,double x1,double y1,
    ...
    double m01 = (y1-y0)/(x1-x0); // b: y - y1 = m( x - x1 ), x = 0
    double b01 = m01 * -x1 + y1;
    double m02, m12, b02, b12;
    m02 = (y2-y0)/(x2-x0);
    m12 = (y2-y1)/(x2-x1);
    b02 = m02 * -x2 + y2;
    b12 = m12 * -x2 + y2; // vertical line checks
...
// get input
    printf("Please enter the following in format 'x,y':\n");
    printf("Triangle Vertex A (x,y): \n"); 
        aggregateVal += scanf("%lf,%lf", &x0,&y0);
    printf("Triangle Vertex B (x,y): \n"); 
        aggregateVal += scanf("%lf,%lf", &x1,&y1);
    printf("Triangle Vertex C (x,y): \n"); 
        aggregateVal += scanf("%lf,%lf", &x2,&y2);
    printf("Test Point (x,y): \n"); 
        aggregateVal += scanf("%lf,%lf", &px,&py);
 
// print error
   if( aggregateVal != 8 ) {
        printf("You're stupid and ");
        printf("didn't put in the right inputs!\n");
        return 1;
    }
