...
int connections(Map g, LocationID start, LocationID end, TransportID type[])
{
   ...
   VList s,bConn;
   for (s = g->connections[start]; s != NULL; s = s->next) {
      //printf("%s %s %d\n",idToName(s->v), idToName(end), idToType(s->v));
      if(s->type == BOAT && idToType(start)!= SEA && idToType(end)!=SEA){ 
      //port city!
         sea = s->v;
         for (bConn = g->connections[sea]; bConn != NULL; bConn = bConn->next){
            //check if end city connected to same sea
            if (bConn->v == end){ 
               join = 1;
            }
         }
      ...
      if (s->v == end) {
         switch (s->type) {
            case ROAD: type[n] = ROAD; break;
            case RAIL: type[n] = RAIL; break;
            case BOAT: type[n] = BOAT; break;
            default:   type[n] = 0; break;
         }
         n++;
         c++;
      }
