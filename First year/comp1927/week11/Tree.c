...
static int cmp(Key i, Link b);
static int less(Key i, Key k, Link b);

int cmp(Key i, Link b){
	b->within->ncompares++;
	return (i-(b->value));
}

int less(Key i, Key k, Link b){
	b->within->ncompares++;
	return (i-k<0);
}
int eq(Link b, Key i){
...
Link insertSplay(Link t, Item it)
{
	...
	if (diff == 0)
		...
		if (less(v,key(t->left->value),t->left)) {
		...
		if (less(key(t->right->value),v,t->right)) {
		...
		*found = 1; // item found, store true  
		...
		if (t->left == NULL){
			...
			*found = 1;
			...
			if (less(k,key(t->left->value),t->left)) {
				// left-left
		...
		else if (eq(t->right,k)) {
			*found = 1;
			res = rotateL(t);
		}
		else{
			if (less(key(t->right->value),k,t->right)) {
				/* right-right */
