...
LocationID *connections(Map g, LocationID start, int size, TransportID type[],
                              int road, int rail, int sea, int dice)
{                    
   assert(g != NULL);

   int i = 1; //index of array, start at 1 bc 0 stores starting location
   int duplicate, n;

   for (s = g->connections[start]; s != NULL; s = s->next) {
      duplicate = 0;
      for (n=0; n<size; n++){
         if(s->v == t[n]) duplicate = 1;
      }
      if (duplicate != 1){
         if(road = 1 && s->type == ROAD){
            t[i] = s->v;
            i++;
         }
         if(sea = 1 && s->type == BOAT){
            t[i] = s->v;
            i++;
         }
      }
